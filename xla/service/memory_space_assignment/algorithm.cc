...
// Total length covered by a changing set of byte ranges: a segment tree over
// the elementary segments between the sorted range bounds, where each node
// keeps how many ranges cover it whole and the covered length below it.
class CoverageTree {
 public:
  // `bounds` are the sorted, distinct range bounds. Elementary segment i is
  // [bounds[i], bounds[i + 1]).
  explicit CoverageTree(std::vector<int64_t> bounds)
      : bounds_(std::move(bounds)),
        cover_(4 * bounds_.size(), 0),
        covered_length_(4 * bounds_.size(), 0) {}

  // Adds `delta` to the cover count of the elementary segments in
  // [first, last).
  void Add(int64_t first, int64_t last, int64_t delta) {
    if (first < last) {
      Add(/*node=*/1, /*lo=*/0, /*hi=*/num_segments(), first, last, delta);
    }
  }

  // Total length of the elementary segments covered by at least one range.
  int64_t covered_length() const {
    return num_segments() > 0 ? covered_length_[1] : 0;
  }

 private:
  int64_t num_segments() const {
    return static_cast<int64_t>(bounds_.size()) - 1;
  }

  // `node` spans the elementary segments [lo, hi), which intersect
  // [first, last).
  void Add(int64_t node, int64_t lo, int64_t hi, int64_t first, int64_t last,
           int64_t delta) {
    if (first <= lo && hi <= last) {
      cover_[node] += delta;
    } else {
      ...
      if (first < mid) {
        Add(2 * node, lo, mid, first, last, delta);
      }
      if (mid < last) {
        Add(2 * node + 1, mid, hi, first, last, delta);
      }
    }
    if (cover_[node] > 0) {
      covered_length_[node] = bounds_[hi] - bounds_[lo];
    } else if (hi - lo == 1) {
      covered_length_[node] = 0;
    } else {
      covered_length_[node] =
          covered_length_[2 * node] + covered_length_[2 * node + 1];
    }
  }

  std::vector<int64_t> bounds_;
  std::vector<int64_t> cover_;
  std::vector<int64_t> covered_length_;
};
...
void MsaAlgorithm::CommittedBytes::AssignUnion(absl::Span<const Range> ranges) {
  ...
  CoverageTree coverage(std::move(bounds));
  auto event = events.begin();
  for (int64_t time = 0; time < num_times(); ++time) {
    for (; event != events.end() && event->time <= time; ++event) {
      coverage.Add(event->first, event->last, event->delta);
    }
    bytes_[time] = coverage.covered_length();
  }
