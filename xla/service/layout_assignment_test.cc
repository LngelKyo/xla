...
// predicate UpdateLayout applies to the materialized operand shape, which
// compares minor_to_major only.
...
ENTRY e {
  ...
  // The early return relies on the operand keeping the shape the constraint
  // was built for; a debug build checks that invariant. The module is put
  // back in order afterwards, since it is verified on destruction.
  HloInstruction* p0 = add->mutable_operand(0);
  HloInstruction* other =
      add->parent()->AddInstruction(HloInstruction::CreateTranspose(
          ShapeUtil::MakeShape(F32, {8, 4}), add->mutable_operand(1), {1, 0}));
  ASSERT_OK(add->ReplaceOperandWithDifferentShape(0, other));
  EXPECT_DEBUG_DEATH(
      layout_assignment
          .SetArrayOperandLayout(LayoutUtil::MakeLayout({0, 1}), add,
                                 /*operand_no=*/0, /*mandatory=*/true,
                                 /*dfs=*/false, /*priority=*/5)
          .IgnoreError(),
      "constraint was built for");
  ASSERT_OK(add->ReplaceOperandWithDifferentShape(0, p0));
  ASSERT_OK(add->parent()->RemoveInstruction(other));
}
