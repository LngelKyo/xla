...
class CallGraphNode {
 ...
 private:
  ...
  // Position of this node in a preorder traversal of the dominator tree of the
  // call graph, and the position after its last descendant. Node 'a' dominates
  // node 'b' iff a.dominator_preorder_ <= b.dominator_preorder_ <
  // a.dominator_preorder_end_.
  int64_t dominator_preorder_ = -1;
  int64_t dominator_preorder_end_ = -1;
...
class CallGraph {
 public:
  ...
  // 'a'. Trivially, a computation dominates itself. 'b' must be in the call
  // graph; a computation 'a' outside the call graph dominates nothing but
  // itself.
 ...
 private:
  ...
  // Computes the dominator tree of the graph and stores its preorder numbering
  // in the nodes. Requires the node depths.
  void SetDominatorTree();
