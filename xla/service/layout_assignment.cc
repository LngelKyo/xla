...
bool OperandLayoutConstraint::IsSatisfiedBy(const Layout& layout) const {
  // The layout half of shape_layout().MatchesLayoutInShape(operand shape with
  // `layout`, /*minor_to_major_only=*/true). The shape half always holds: the
  // constructor builds the constraint from the operand's shape, with a
  // layout, and the caller checks that the operand still has that shape.
  return Layout::Equal()
      .IgnoreTiles()
      .IgnoreElementSize()
      .IgnoreMemorySpace()
...
absl::Status LayoutAssignment::SetArrayOperandLayout(
  ...
  if (constraint_slot != nullptr) {
    // The slot was built from this operand's shape, and operand lists do not
    // change while the slot is alive (ClearPreviousPassSideEffects resets the
    // slots each round, copies are inserted after propagation). So the shape
    // half of the UpdateLayout check holds and only the layout is compared.
    DCHECK(Shape::Equal().IgnoreDynamicDimension().IgnoreLayout()(
        operand->shape(), constraint_slot->shape_layout().shape()))
        << "operand " << operand_no << " of " << instruction->ToString()
        << " has shape " << operand->shape()
        << " but its constraint was built for "
        << constraint_slot->shape_layout().shape();
    if (constraint_slot->IsSatisfiedBy(layout)) {
      return absl::OkStatus();
    }
  }
